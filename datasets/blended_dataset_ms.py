...
class BlendedMVSDataset(Dataset):
    def __init__(self, datapath, listfile, mode, nviews, ndepths=192, interval_scale=1.06, random_crop=False, augment=False,
        ...
        self.mono_depths_path = kwargs.get("mono_depths_path")
        ...
        self.scales = self.multi_scale_args['scales']
        self.scales_epoch = self.scales.copy()
        self.resize_range = self.multi_scale_args['resize_range']
        self.batch_size = kwargs.get('batch_size', 2)
        self.world_size = kwargs.get('world_size', 1)
        self.max_crop_attempts = int(kwargs.get('max_crop_attempts', 20))
        self.io_retry_count = int(kwargs.get('io_retry_count', 60))
        self.io_retry_sleep = float(kwargs.get('io_retry_sleep', 0.2))
        self.img_size_map = []
    ...
    def _resolve_mono_depth(self, scan, view_id):
        name = f"{view_id:08d}.npy"
        candidates = []
        if self.mono_depths_path:
            candidates.extend([
                os.path.join(self.mono_depths_path, scan, "mono_depth", name),
                os.path.join(self.mono_depths_path, scan, name),
                os.path.join(self.mono_depths_path, name),
            ])
        candidates.append(
            os.path.join(
                self.datapath, scan, scan, scan, "mono_depth", name
            )
        )
    ...
    @staticmethod
    def _retryable_io_error(err):
        if isinstance(err, BlockingIOError):
            return True
        if isinstance(err, OSError):
            if err.errno in (5, 11, 35):
                return True
            if err.errno is None and "Resource temporarily unavailable" in str(err):
                return True
        return False

    def _run_with_retry(self, op, filename):
        tries = max(1, self.io_retry_count)
        last_err = None
        for attempt in range(tries):
            try:
                return op()
            except Exception as err:
                if not self._retryable_io_error(err):
                    raise
                last_err = err
                if attempt + 1 < tries:
                    time.sleep(self.io_retry_sleep * (attempt + 1))
        raise last_err
    ...
    def read_cam_file(self, filename):
        ...
        lines = self._run_with_retry(_read, filename)
    ...
    def read_img(self, filename):
        return self._run_with_retry(lambda: Image.open(filename).convert('RGB').copy(), filename)

    def read_depth(self, filename):
        # read pfm depth file
        return self._run_with_retry(
            lambda: np.array(read_pfm(filename)[0], dtype=np.float32),
            filename,
        )

    def read_depth_hr(self, filename):
        # read pfm depth file
        depth = self._run_with_retry(
            lambda: np.array(read_pfm(filename)[0], dtype=np.float32),
            filename,
        )
    ...
    def pre_resize(self, img, depth, mono_depth, intrinsic, mask, resize_scale):
        ...
        if mono_depth is not None:
            mono_depth = cv2.resize(mono_depth, (int(ori_w * resize_scale), int(ori_h * resize_scale)), interpolation=cv2.INTER_NEAREST)

        if mask is not None:
            mask = cv2.resize(mask, (int(ori_w * resize_scale), int(ori_h * resize_scale)), interpolation=cv2.INTER_NEAREST)

        return img, depth, mono_depth, output_intrinsics, mask

    def final_crop(self, img, depth,mono_depth, intrinsic, mask, crop_h, crop_w, offset_y=None, offset_x=None):
        h, w, _ = img.shape
        ...
        if mono_depth is not None:
            cropped_mono_depth = mono_depth[offset_y:offset_y + crop_h, offset_x:offset_x + crop_w]
        else:
            cropped_mono_depth = None
        ...
        return cropped_image, cropped_depth,cropped_mono_depth, output_intrinsics, cropped_mask
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            if i == 0:
                ...
                mono_depth_filename = self._resolve_mono_depth(scan, vid)
                if mono_depth_filename is None:
                    mono_depth = None
                else:
                    mono_depth = self._run_with_retry(
                        lambda: np.load(mono_depth_filename),
                        mono_depth_filename,
                    ).astype(np.float32)
            else:
                depth_hr = None
            ...
            # Choose the resize range from the crop size.
            if self.mode == 'train':
            ...
            img = np.asarray(img)
            if resize_scale != 1.0:
                img, depth_hr,mono_depth, intrinsics, depth_mask_hr = self.pre_resize(img, depth_hr,mono_depth,intrinsics, depth_mask_hr, resize_scale)

            if i == 0:  # reference view
                valid_crop = False
                for _ in range(self.max_crop_attempts):
                    img_, depth_hr_,mono_depth_, intrinsics_, depth_mask_hr_ = self.final_crop(img, depth_hr,mono_depth, intrinsics, depth_mask_hr,
                                                                                                       crop_h=crop_h, crop_w=crop_w)
                    mask_read_ms_ = self.generate_stage_depth(depth_mask_hr_)
                    valid_crop = self.mode != 'train' or np.any(mask_read_ms_['stage1'] > 0.0) or not self.random_crop
                    if valid_crop:
                        break

                if not valid_crop:
                    h_img, w_img = img.shape[:2]
                    center_y = max((h_img - crop_h) // 2, 0)
                    center_x = max((w_img - crop_w) // 2, 0)
                    img_, depth_hr_, mono_depth_, intrinsics_, depth_mask_hr_ = self.final_crop(
                        img, depth_hr, mono_depth, intrinsics, depth_mask_hr,
                        crop_h=crop_h, crop_w=crop_w, offset_y=center_y, offset_x=center_x
                    )
                    mask_read_ms_ = self.generate_stage_depth(depth_mask_hr_)

                depth_ms = self.generate_stage_depth(depth_hr_)
                mono_depth_ms = (
                    self.generate_stage_depth(mono_depth_)
                    if mono_depth_ is not None
                    else None
                )
                mask = mask_read_ms_
                img = img_
                ...
                img, depth_hr,mono_depth, intrinsics, depth_mask_hr = self.final_crop(img, depth_hr,mono_depth, intrinsics, depth_mask_hr,
                                                                                     crop_h=crop_h, crop_w=crop_w)
        ...
        if mono_depth_ms is not None:
            sample["mono_depth"] = mono_depth_ms
        return sample
