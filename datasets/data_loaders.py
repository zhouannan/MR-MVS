...
from .mvs_dataset_all import MVSDatasetAll

np.random.seed(1234)

class DTULoader(DataLoader):
    def __init__(self, data_path, data_list, mode, num_srcs,
                 mono_depths_path=None, num_depths=192, interval_scale=1.0,
        ...
        if (mode == 'train') or (mode == 'val'):
            ...
            self.mvs_dataset = DTUMVSDataset(data_path, data_list, mode, num_srcs, num_depths, interval_scale,
                                             ...
                                             augment=augment, aug_args=aug_args, refine=refine,
                                             height=height, width=width,
                                             mono_depths_path=mono_depths_path, **kwargs)
        elif mode=='test':
            self.mvs_dataset = MVSDataset(data_path, data_list, mode, num_srcs, mono_depths_path, num_depths, interval_scale,
                                          shuffle=shuffle, batch_size=batch_size,
                                          max_h=max_h, max_w=max_w, fix_res=fix_res, dataset=dataset_eval, refine=refine,
                                          iterative=iterative, **kwargs)
        else:
            self.mvs_dataset = MVSDatasetAll(data_path, data_list, mode, num_srcs, mono_depths_path, num_depths, interval_scale,
                                shuffle=shuffle, batch_size=batch_size,
                                max_h=max_h, max_w=max_w, fix_res=fix_res, dataset=dataset_eval, refine=refine,
                                iterative=iterative, **kwargs)
