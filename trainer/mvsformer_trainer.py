    ...
    def _train_epoch(self, epoch):
        ...
        if self.multi_scale:
            for i in range(len(self.data_loader)):
                if self.train_sampler is not None:
                    self.data_loader[i].dataset.reset_dataset(
                        self.train_sampler
                    )
        ...
        for dl in self.data_loader:
            ...
            for batch_idx, sample in enumerate(t_loader):
                ...
                mono_depth_ms = sample_cuda.get("mono_depth")
                ...
                for bi in range(iters_to_accumulate):
                    ...
                    if mono_depth_ms is not None:
                        mono_depth_ms_tmp = {
                            ...
                            for key, value in mono_depth_ms.items()
                        }
                    ...
                    if self.fp16:
                        with torch.cuda.amp.autocast(dtype=torch.bfloat16 if self.bf16 else torch.float16):
                            ...
                            outputs = self.model.forward(
                                imgs_tmp,
                                cam_params_tmp,
                                depth_values[b_start:b_end],
                                mono_depth=mono_depth_ms_tmp,
                            )
                    else:
                ...
                if self.fp16:
                    ...
                    if new_scale == pre_scale:  # Optimization kept the scale.
            ...
            for val_data_idx, dl in enumerate(self.valid_data_loader):
                ...
                for batch_idx, sample in enumerate(tqdm(dl)):
                    ...
                    if self.rank == 0 and val_data_idx == 0:
                        # Save one visualization per scan.
