...
def mvsnet_to_gipuma(dense_folder, gipuma_point_folder):
    ...
    image_names = [i for i in image_names if not i.startswith('.')]
    for image_name in image_names:
        image_prefix = os.path.splitext(image_name)[0]
        in_cam_file = os.path.join(cam_folder, image_prefix + '_cam.txt')
        out_cam_file = os.path.join(gipuma_cam_folder, image_name.replace("jpg", "png") + '.P')
    ...
    for image_name in image_names:
        ...
        out_image_file = os.path.join(gipuma_image_folder, image_name.replace("jpg", "png"))
        shutil.copy(in_image_file, out_image_file)

    # convert depth maps and fake normal maps
    gipuma_prefix = '2333__'
    for image_name in tqdm(image_names, desc='convert depth maps'):
        image_prefix = os.path.splitext(image_name)[0]
...
def probability_filter(dense_folder, prob_threshold):
    confidence_folder = os.path.join(dense_folder, 'confidence')

    # convert cameras
    confidence_names = os.listdir(confidence_folder)
    confidence_names = [i for i in confidence_names if not i.startswith('.')]
    for confidence_name in tqdm(confidence_names, desc='filtering'):
        image_prefix = os.path.splitext(confidence_name)[0]
        init_depth_map_path = os.path.join(dense_folder, "depth_est", image_prefix + '.pfm')
        init_depth_npy_path = os.path.join(dense_folder, "depth_est", image_prefix + '.npy')
        prob_map_path = os.path.join(dense_folder, "confidence", image_prefix + '.npy')
        ...
        elif os.path.exists(init_depth_npy_path):
            depth_map = np.load(init_depth_npy_path).astype(np.float32)
        else:
            raise FileNotFoundError(
                f"Missing both depth map files: {init_depth_map_path} and {init_depth_npy_path}"
            )
...
def depth_map_fusion(point_folder, fusibile_exe_path, disp_thresh, num_consistent, color=True):
