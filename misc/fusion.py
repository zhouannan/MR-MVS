...
def get_pixel_grids(height, width, device=None):
    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    x_coord = (torch.arange(width, dtype=torch.float32, device=device) + 0.5).repeat(height, 1)
    y_coord = (torch.arange(height, dtype=torch.float32, device=device) + 0.5).repeat(width, 1).t()
...
def project_img(src_img, dst_depth, src_cam, dst_cam, height=None, width=None):  # nchw, n1hw -> nchw, n1hw
    ...
    dst_idx_img_homo = get_pixel_grids(height, width, src_img.device).unsqueeze(0)  # nhw31
...
def get_reproj(ref_depth, srcs_depth, ref_cam, srcs_cam):  # n1hw, nv1hw -> n1hw
    ...
    idx_img = get_pixel_grids(h, w, srcs_depth.device).unsqueeze(0)  # 1hw31
...
def vis_filter(ref_depth, reproj_xyd, in_range, img_dist_thresh, depth_thresh, vthresh):
    ...
    xy = get_pixel_grids(h, w, reproj_xyd.device).permute(3, 2, 0, 1).unsqueeze(1)[:, :, :2]  # 112hw
...
def get_reproj_dynamic(ref_depth, srcs_depth, ref_cam, srcs_cam):  # n1hw, nv1hw -> n1hw
    ...
    idx_img = get_pixel_grids(h, w, ref_depth.device).unsqueeze(0)  # 1hw31  # [1,h,w,3,1]
...
def vis_filter_dynamic(ref_depth, reproj_xyd, dist_base=4, rel_diff_base=1300):
    ...
    n, v, _, h, w = reproj_xyd.size()
    xy = get_pixel_grids(h, w, device).permute(3, 2, 0, 1).unsqueeze(1)[:, :, :2]  # 112hw
